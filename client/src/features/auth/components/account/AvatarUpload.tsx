...
import { deleteAvatar, uploadAvatar } from '../../services/auth-api';
...
const AvatarUpload: React.FC<AvatarUploadProps> = ({ avatarURL, targetUserId }) => {
    ...
    return (
        <div className="flex flex-col items-center gap-4">
            {/* Avatar Display */}
            <div
                className="w-32 h-32 border rounded-lg border-gray-300 overflow-hidden cursor-pointer shadow-sm transition hover:ring-1 hover:ring-primary"
            ...
            <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)}>
                <div className="flex flex-col items-center gap-6 p-4 md:p-6">
                    {/* Upload Card */}
                    <div className="w-full max-w-md border border-gray-200 rounded-2xl p-6 bg-white shadow-lg flex flex-col items-center gap-6 text-center">
                        ...
                        <label
                            ...
                            className="cursor-pointer bg-primary text-white px-4 py-2 rounded-md hover:bg-white hover:text-primary border border-gray-300 text-sm transition"
                        >
                            Choose Image
                        </label>
                        ...
                        {/* Cropper */}
                        {imageSrc && (
                            <div className="relative w-64 h-64 rounded-md overflow-hidden border border-gray-300 bg-black shadow">
                                <Cropper
                                    image={imageSrc}
                                    crop={crop}
                                    zoom={zoom}
                        ...
                        {imageSrc && (
                            <div className="w-full flex items-center gap-2">
                                <label htmlFor="zoom" className="text-sm text-gray-600 w-16">
                                    Zoom
                                </label>
                                <input
                                    id="zoom"
