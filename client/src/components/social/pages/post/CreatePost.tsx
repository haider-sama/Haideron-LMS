...
import { CreatePostPayload } from "../../../../constants/social/interfaces";
import { PostTypeEnum } from "../../../../../../server/src/shared/social.enums";
import { createPost } from "../../../../api/social/post/post-api";
...
export const CreatePost: React.FC<AddPostProps> = ({ forumId, onClose }) => {
    ...
    const mutation = useMutation<{ message: string }, Error, CreatePostPayload>({
        mutationFn: createPost,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["posts", forumId] });
            toast.success("Post created!");
            setForm({ forumId, type: PostTypeEnum.TEXT, content: "", mediaUrls: [] });
            onClose?.();
        },
        onError: (err) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const payload: CreatePostPayload = { ...form };
    ...
    return (
        <div className="flex justify-center px-4">
            <form onSubmit={handleSubmit} className="w-full max-w-xl p-4 space-y-4 bg-white dark:bg-darkCard rounded-xl relative">
                ...
                {((form.mediaUrls?.length || 0) > 0 || uploading) && (
                        ...
                        <div className="flex flex-wrap gap-2">
                            {form.mediaUrls?.map((url, idx) => (
                                <div key={idx} className="relative w-32 h-32">
                                    <LazyMedia src={url} alt={`preview-${idx}`} isSingle />
                                    <button
                                        type="button"
