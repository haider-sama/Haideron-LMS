...
import { FilterPostsResponse, Post, UpdatePostPayload } from "../../../../constants/social/interfaces";
import { useToast } from "../../../../context/ToastContext";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { PostTypeEnum } from "../../../../../../server/src/shared/social.enums";
import { deleteMediaByUrl, uploadMedia } from "../../../../lib/mediaUploader";
import { updatePost } from "../../../../api/social/post/post-api";
...
export const EditPost: React.FC<EditPostProps> = ({ post, onClose }) => {
    ...
    const mutation = useMutation({
        ...
        onMutate: async (payload) => {
            ...
            await queryClient.cancelQueries({ queryKey: ["forumPosts", post.forumId] });

            // Snapshot the previous value
            const previousData = queryClient.getQueryData<{ posts: Post[] }>(["forumPosts", post.forumId]);

            // Optimistically update the post in cache
            queryClient.setQueryData<FilterPostsResponse>(["forumPosts", post.forumId], (old) => {
                if (!old) return old;

                return {
                    ...old,
                    posts: old.posts.map((p: Post) =>
                        p.id === post.id ? { ...p, ...payload } : p
                    ),
                };
            });
        ...
        onError: (err: any, _payload, context) => {
            ...
            if (context?.previousData) {
                queryClient.setQueryData(["forumPosts", post.forumId], context.previousData);
            }
            toast.error(err.message || "Failed to update post");
        },
        onSettled: () => {
            // Ensure the cache is fresh
            queryClient.invalidateQueries({ queryKey: ["forumPosts", post.forumId] });
        },
        onSuccess: () => {
            ...
            <form onSubmit={handleSubmit} className="w-full max-w-xl p-4 space-y-4 bg-white dark:bg-darkCard rounded-xl relative">
                ...
                <div className="flex justify-end flex-wrap items-center gap-x-3 gap-y-2 pt-6 border-t dark:border-gray-700 mt-6">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-5 py-2 text-sm font-medium rounded-xl text-white shadow-md transition 
                            ${mutation.isPending
